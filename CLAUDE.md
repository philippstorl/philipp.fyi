...
- **`src/assets/` holds standalone images imported directly via `astro:assets`** (e.g. `import heroFigure from '@/assets/hero-figure.png'`), for images that aren't part of a content collection. This is distinct from `src/content/*/`'s images, which are colocated with their MDX/Markdown and validated through the `image()` schema helper in `src/content.config.ts` — don't move a collection's images into `src/assets/` or vice versa.
- **For local (statically-imported) `<Image>` sources, pass only `width` or only `height`, not both** — Astro infers the other dimension from the source file's current aspect ratio, so the displayed image can't silently distort if the source is later replaced at a different aspect ratio. Passing both pins the rendered aspect ratio independently of the file, which is only what you want when deliberately forcing a crop (e.g. `WorkCard.astro`'s fixed-aspect `object-cover` thumbnails).
- **A PNG that looks transparent in an editor or chat preview may not actually have an alpha channel** — AI image generators in particular often render a checkerboard pattern as literal flat pixels rather than real transparency. Verify with `sips -g all <file>` (look for `hasAlpha: yes`) before assuming a "transparent" image will adapt to dark mode — otherwise it shows up as a visible solid-color box once placed on a `dark:`-aware background.
- **Not every image needs to adapt to the dark/light toggle.** A flat graphic with its own baked-in colors (e.g. `Hero.astro`'s `hero-figure.png`, a designed package-style illustration) can be left exactly as-is — don't build theme-conditional logic or a dark-mode variant for it. Instead wrap it in a `border border-border bg-card rounded-xl p-*` frame (the same tokens used for `WorkCard.astro`/`AboutSection.astro` cards, which already flip with `.dark`) so the frame reads as intentional on either background while the artwork itself stays untouched.
