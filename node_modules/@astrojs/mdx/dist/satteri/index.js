...
function createMdxProcessor(mdxOptions, satteriOptions, ctx) {
  ...
  function initShiki() {
    ...
    if (syntaxHighlightType === "shiki") {
      ...
      initPromise = createShikiHighlighter({
        ...
        highlightFn = (code, lang, meta) => hl.codeToHtml(code, lang, {
          meta,
  ...
  return {
    async process(content, filePath, frontmatter) {
      ...
      const allMdastPlugins = satteriOptions.mdastPlugins.length ? [collectImages, ...satteriOptions.mdastPlugins] : [collectImages];
      const hastPlugins = [];
      if (highlightFn) {
