...
// The three distinct grid-column shapes reused across this case study's
// figures — named once so every call site referencing the same shape can't
// silently drift out of sync with its sibling copies the way repeated
// inline array literals could. Kept contiguous with the imports above (no
// blank line) — MDX's ESM-block parser stops treating subsequent statements
// as real JS the moment a blank line separates them from the import block,
// after which `{ minWidth: 640, ... }` gets misread as a JSX expression
// container instead of an object literal (confirmed by bisecting a minimal
// repro against a real `astro build`, not just typecheck, which stayed
// green throughout since MDX-body parsing is a separate, stricter pipeline).
export const TWO_COL_TIERS = [{ minWidth: 640, columns: 2 }, { columns: 1 }]
export const THREE_COL_TIERS = [
    { minWidth: 1024, columns: 3 },
    { minWidth: 640, columns: 2 },
    { columns: 1 },
]
export const REGIONAL_TIERS = [{ minWidth: 640, columns: 3 }, { columns: 1 }]
...
<div class="my-8 grid grid-cols-1 gap-4 sm:grid-cols-2">
    <ResponsiveFigure
        ...
        caption="Before - April 2020, plain HTML, agency-maintained"
        {...responsiveGridFigureSizing(TWO_COL_TIERS)}
    />
    <ResponsiveFigure
        src={voices2020}
        alt="VOICES website homepage in October 2020, the first Eleventy build"
        caption="After - October 2020, the first Eleventy build"
        {...responsiveGridFigureSizing(TWO_COL_TIERS)}
    />
</div>
...
<div class="my-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
    <ResponsiveFigure
        ...
        caption="2020"
        {...responsiveGridFigureSizing(THREE_COL_TIERS)}
    />
    <ResponsiveFigure
        src={voices2022}
        alt="VOICES website homepage after the 2022 rebrand"
        caption="2022 - first rebrand"
        {...responsiveGridFigureSizing(THREE_COL_TIERS)}
    />
    <ResponsiveFigure
        src={voices2023}
    ...
    <ResponsiveFigure
        ...
        caption="2025 - second rebrand"
        {...responsiveGridFigureSizing(THREE_COL_TIERS)}
    />
    <ResponsiveFigure
        src={voices2026}
        alt="VOICES website homepage in 2026, current state"
        caption="2026 - current state"
        {...responsiveGridFigureSizing(THREE_COL_TIERS)}
    />
</div>
...
<div class="my-6 flex flex-col justify-center gap-4 sm:flex-row">
    <ResponsiveFigure
        ...
        {...fixedWidthFigureSizing(320, { siblings: 2, breakpoint: 640 })}
        class="max-w-xs"
...
<div class="my-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
    <ResponsiveFigure
        ...
        caption="Berlin - in-person, German"
        {...responsiveGridFigureSizing(REGIONAL_TIERS)}
    />
    <ResponsiveFigure
        src={voices2026En}
        alt="VOICES Americas (NA) homepage, 2026"
        caption="Americas - virtual"
        {...responsiveGridFigureSizing(REGIONAL_TIERS)}
    />
    <ResponsiveFigure
        src={voices2026Emea}
        alt="VOICES London (EMEA) homepage, 2026"
        caption="London - in-person"
        {...responsiveGridFigureSizing(REGIONAL_TIERS)}
    />
</div>
<ResponsiveFigure
    ...
    caption="The hub page that routes to each regional edition"
    {...fullWidthFigureSizing()}
    class="my-6"
/>
...
<ResponsiveFigure
    ...
    caption="The multi-track agenda component, populated for VOICES Americas 2026"
    {...fullWidthFigureSizing()}
    class="my-6"
/>
...
<ResponsiveFigure
    ...
    caption="The after-event state - recordings populated automatically from YouTube"
    {...fullWidthFigureSizing()}
    class="my-6"
/>
