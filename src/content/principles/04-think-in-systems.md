---
title: "Think in systems, communicate the why"
description: "Most problems are symptoms of something structural. Finding the root cause, and explaining it clearly, is the only way to stop the same issue coming back."
---

Most problems I've encountered weren't isolated incidents. They were symptoms of something structural. The antidote, every time, has been to step back and find the root cause rather than patching the visible thing and moving on.

But identifying a root cause isn't enough; you have to communicate it in a way that people can act on. I once had to explain to a colleague on a completely different team why certain website templates were locked and couldn't be freely edited. I didn't just say "because policy." I explained the history: _"We are coming from a time where we had a mess because everyone was able to do everything; we had no control over the brand and what was in use."_ The quality gate I introduced for go-lives got the same treatment: not just the rule, but the reasoning behind it. Every bug fix closed the loop with screenshots and before-and-after evidence. The request intake system I built channeled ad-hoc requests into a predictable process instead of letting them overwhelm the team, and the sprint framework did the same for unpredictable delivery.

People follow standards more readily when they understand why those standards exist. And systems improve faster when people understand the root cause, not just the symptom.
