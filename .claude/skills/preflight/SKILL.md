...
8. `npm run check:font-preloads`, `npm run check:inline-module-scripts`, `npm run check:page-coverage` and `npm run check:csp` — run whenever step 7 ran; they check the fresh `dist/`. CI runs all four on every build, so skipping them locally can let a CI failure through.
