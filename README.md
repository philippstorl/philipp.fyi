...
| `e2e/navigation.spec.ts`      | Header, nav links (Work, About, Principles, Contact), name mark, theme toggle, skip-to-main-content link, mobile nav closes on Escape (mobile project only) and on click outside                                                                                         |
| `e2e/work.spec.ts`            | All 4 case study pages render, card links resolve correctly, og:type is article with article:published_time; Work index page heading/cards, interlinks to About/Principles/Recommendations                                                                               |
...
| `e2e/contact-form.spec.ts`    | Contact form validation errors + focus, invalid-email message, single summary alert (not one per field) on multi-field failure, summary re-announces on a same-count retry, mocked failed submission (error banner), mocked successful submission (confirmation + focus) |
